...
Five pages: Overview, Network, Power, System and Terminal. The terminal page consumes lines written to `/tmp/openoledui-terminal`.
...
```bash
ping -c 4 1.1.1.1 > /tmp/openoledui-terminal
```

Raspberry Pi 5 has a dedicated power button with shutdown/wake semantics, so OpenOledUI leaves it to the OS. Product enclosures should connect separate momentary GPIO buttons and translate them to `Key::Prev`, `Key::Next` and `Key::Select`.
