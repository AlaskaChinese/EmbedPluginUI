...
struct AppState {
    ...
    AppState() { ui.add_page(home); ui.add_page(sensors); ui.add_page(about); }
};
...
void handle_key(Ui& ui, KeySym key) {
    ...
    switch (key) {
        case XK_Right: case XK_d: case XK_D: ui.handle(Key::Next, now); break;
        case XK_Left: case XK_a: case XK_A: ui.handle(Key::Prev, now); break;
        case XK_Return: case XK_space: ui.handle(Key::Select, now); break;
        case XK_Escape: ui.handle(Key::Back, now); break;
        default: break;
    }
}
...
void draw_frame(Display* display, Window window, GC gc, AppState& app) {
    ...
    XFillRectangle(display, window, gc, kPad - 8, kPad - 8, Canvas::Width * kScale + 16, Canvas::Height * kScale + 16);
    ...
    for (int y = 0; y < Canvas::Height; ++y) {
        for (int x = 0; x < Canvas::Width; ++x) {
            ...
            if (((fb[index] >> (y & 7)) & 1u) != 0u) {
                XFillRectangle(display, window, gc, kPad + x * kScale, kPad + y * kScale, kScale, kScale);
            }
        }
    }
...
int main() {
    ...
    Window window = XCreateSimpleWindow(display, RootWindow(display, screen), 120, 120, kWidth, kHeight, 0, BlackPixel(display, screen), BlackPixel(display, screen));
    XStoreName(display, window, "EmbedPluginUI - 128x64 Simulator");
    ...
    while (running) {
        while (XPending(display) > 0) {
            ...
            if (event.type == KeyPress) handle_key(app.ui, XLookupKeysym(&event.xkey, 0));
            else if (event.type == ClientMessage && static_cast<Atom>(event.xclient.data.l[0]) == wm_delete) running = false;
        }
