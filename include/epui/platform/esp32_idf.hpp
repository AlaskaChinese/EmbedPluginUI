...
class Esp32IdfPlugin final : public PlatformPlugin {
public:
    explicit Esp32IdfPlugin(Esp32IdfHooks hooks, std::uint8_t oled_address = 0x3C,
                            const char* plugin_name = "esp32-idf")
    ...
    static bool gpio_reader(void* user, int pin) {
        return user && static_cast<Esp32IdfPlugin*>(user)->read_gpio(pin);
    }

private:
    Esp32IdfHooks hooks_;
    const char* name_;
